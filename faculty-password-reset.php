...
if(isset($_POST['reset_password_button'])){
  ...
  if(empty($form_error)){
    ...
    if($password_1 != $password_2){
      ...
      try{
        $SQLQuery = "SELECT username FROM faculty WHERE username = :username";
        $statement = $db->prepare($SQLQuery);
        $statement->execute(array(':username' => $username));

        if($statement->rowCount() == 1){
          $hashed_password = password_hash($password_1, PASSWORD_DEFAULT);
          $SQLUpdate = "UPDATE faculty SET password = :password WHERE username = :username";
          $statement = $db->prepare($SQLUpdate);
          $statement->execute(array(':password' => $hashed_password, ':username' => $username));
          $result =  header("location: faculty-login.php");
        }
        else{
